...
(async () => {
  ...
  // ════ Escenario G1: sync de games — reemplazo completo, no aditivo (T-games-1) ════
  // El bloque de sync de games vive dentro de `if (!lightMode)`, y `lightMode`
  // es en realidad el parámetro `silent` de tryAutoSync reenviado a doSync
  // (`doSync(silent)`). Todos los escenarios anteriores llaman
  // tryAutoSync(true) — modo silencioso —, lo que de paso saltaba siempre
  // este bloque: por eso no tenía cobertura. Aquí se usa tryAutoSync(false)
  // a propósito para ejercitarlo.
  let W = freshDevice('G1');

  // Estado inicial en el servidor: 2 colecciones, 3 partidas.
  let admRes = await (await fetch(`${SERVER}/admin/import_games`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      game_collections: [{ name: 'colA', folder: 'colA' }, { name: 'colB', folder: 'colB' }],
      games: [
  ...
  check('G1: import inicial en servidor OK', admRes.collections === 2 && admRes.games === 3, JSON.stringify(admRes));

  let g1ok = await W.runAsync('return await tryAutoSync(false)');
  check('G1: sync tras import inicial ok', g1ok === true);
  check('G1: cliente recibe las 2 colecciones', W.q1('SELECT COUNT(*) c FROM game_collections').c === 2);
  check('G1: cliente recibe las 3 partidas', W.q1('SELECT COUNT(*) c FROM games').c === 3);

  // El "servidor" (script de importación) borra colB y sustituye colA por
  // una partida nueva — simulando que el disco cambió.
  admRes = await (await fetch(`${SERVER}/admin/import_games`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      game_collections: [{ name: 'colA', folder: 'colA' }],
      games: [{ collection_name: 'colA', name: 'g_nuevo.sgf', sgf_path: 'games/colA/g_nuevo.sgf' }],
    }),
  })).json();
  check('G1: re-import en servidor OK', admRes.collections === 1 && admRes.games === 1, JSON.stringify(admRes));

  g1ok = await W.runAsync('return await tryAutoSync(false)');
  check('G1: segundo sync ok', g1ok === true);
  check('G1: cliente refleja el reemplazo — 1 colección (colB desapareció)',
    W.q1('SELECT COUNT(*) c FROM game_collections').c === 1, JSON.stringify(W.q('SELECT name FROM game_collections')));
  check('G1: cliente refleja el reemplazo — 1 partida (solo g_nuevo.sgf)',
    W.q1('SELECT COUNT(*) c FROM games').c === 1 && W.q1('SELECT name FROM games').name === 'g_nuevo.sgf',
    JSON.stringify(W.q('SELECT name FROM games')));
