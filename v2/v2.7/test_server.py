...
# ══ FASE C: /admin/import_games — reemplazo completo (T-games-1) ══
make_db()
p = start_server()
try:
    # Primer import: 2 colecciones, 3 partidas
    st, body, _ = req("/admin/import_games", "POST", {
        "game_collections": [
            {"name": "colA", "folder": "colA"},
            {"name": "colB", "folder": "colB"},
        ],
        "games": [
            {"collection_name": "colA", "name": "g1.sgf", "sgf_path": "games/colA/g1.sgf"},
            {"collection_name": "colA", "name": "g2.sgf", "sgf_path": "games/colA/g2.sgf"},
            {"collection_name": "colB", "name": "g3.sgf", "sgf_path": "games/colB/g3.sgf"},
        ],
    })
    check("import inicial → 200", st == 200, f"st={st} body={body}")
    check("import inicial: 2 colecciones", body.get("collections") == 2, f"{body}")
    check("import inicial: 3 partidas", body.get("games") == 3, f"{body}")

    con = sqlite3.connect(DB); con.row_factory = sqlite3.Row
    n_cols = con.execute("SELECT COUNT(*) c FROM game_collections").fetchone()["c"]
    n_games = con.execute("SELECT COUNT(*) c FROM games").fetchone()["c"]
    check("DB tras import inicial: 2 colecciones", n_cols == 2, f"n={n_cols}")
    check("DB tras import inicial: 3 partidas", n_games == 3, f"n={n_games}")
    con.close()

    # Segundo import: colB desaparece, colA se queda con una partida distinta.
    # Debe ser un REEMPLAZO, no un añadido.
    st, body, _ = req("/admin/import_games", "POST", {
        "game_collections": [{"name": "colA", "folder": "colA"}],
        "games": [
            {"collection_name": "colA", "name": "g_nuevo.sgf", "sgf_path": "games/colA/g_nuevo.sgf"},
        ],
    })
    check("re-import → 200", st == 200, f"st={st} body={body}")
    con = sqlite3.connect(DB); con.row_factory = sqlite3.Row
    cols = [r["name"] for r in con.execute("SELECT name FROM game_collections")]
    games_rows = [r["name"] for r in con.execute("SELECT name FROM games")]
    check("re-import: colB eliminada", cols == ["colA"], f"cols={cols}")
    check("re-import: solo queda g_nuevo.sgf", games_rows == ["g_nuevo.sgf"], f"games={games_rows}")
    con.close()

    # Partida cuya colección no existe en el mismo payload → se omite, no rompe el import
    st, body, _ = req("/admin/import_games", "POST", {
        "game_collections": [{"name": "colX", "folder": "colX"}],
        "games": [
            {"collection_name": "colX", "name": "ok.sgf", "sgf_path": "games/colX/ok.sgf"},
            {"collection_name": "colY_no_existe", "name": "huerfano.sgf", "sgf_path": "games/colY/huerfano.sgf"},
        ],
    })
    check("import con partida huérfana → 200", st == 200, f"st={st} body={body}")
    check("import con partida huérfana: 1 colección", body.get("collections") == 1, f"{body}")
    check("import con partida huérfana: 1 partida insertada", body.get("games") == 1, f"{body}")
    check("import con partida huérfana: 1 partida omitida", body.get("skipped_games") == 1, f"{body}")

    # Validación de payload
    st, body, _ = req("/admin/import_games", "POST", {"game_collections": "no-es-lista", "games": []})
    check("game_collections no es lista → 400", st == 400, f"st={st} body={body}")
    st, body, _ = req("/admin/import_games", "POST", {"game_collections": [{"name": "x"}], "games": []})
    check("colección sin 'folder' → 400", st == 400, f"st={st} body={body}")
    st, body, _ = req("/admin/import_games", "POST", {"game_collections": [], "games": [{"name": "x"}]})
    check("partida incompleta → 400", st == 400, f"st={st} body={body}")
finally:
    stop(p)
