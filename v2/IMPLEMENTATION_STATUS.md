...
- 4.6 BAJA / N.A. — `local_ip` ya no impide arrancar el servidor en hosts sin resolución de hostname. `sgf_path` sin normalizar (pendiente).
- 5.1 Cliente, puntos calientes — P2 corregido (push solo de pendientes) y fase A de R3 (saveDB debounced). P1 completo (DB fría/caliente), P3-P9 no.
...
- 8 BACKEND — ThreadingHTTPServer, logging con niveles, validaciones, dispatch unificado de POST/PUT mediante tablas de rutas, arranque robusto. Sin `schema_version` formal, sin manejo de SIGTERM, sin HTTP/1.1 keep-alive (decisión conservadora: no cambiar semántica de conexión a través de Caddy sin poder probarla en producción).
...
- 3.7 Listeners acumulativos
- 3.8 Runs de review con total=0 inconsistentes
- 3.9 Resize resetea el problema en curso
- 3.12 Otros (resumeRun terminado, bare columns, PK de run_items, resolución de segundo en dedup sin uuid, SFX iOS, AE en setup)
...
- [ ] 1.1  [ ] 1.2  [ ] 1.3  [ ] 1.4  [ ] 1.5
- [ ] 2.1  [ ] 2.2  [ ] 2.3  [ ] 2.4  [ ] 2.5
- [x] 3.1  [x] 3.2  [x] 3.3  [x] 3.4  [ ] 3.5  [ ] 3.6  [ ] 3.7  [ ] 3.8  [ ] 3.9  [~] 3.10  [~] 3.11  [ ] 3.12
- [x] 4.1  [x] 4.2  [x] 4.3  [~] 4.4  [ ] 4.5  [~] 4.6  [ ] 4.7
- [~] 5.1  [~] 5.2  [ ] 5.3  [ ] 5.4
- [ ] 6.1  [~] 6.2  [ ] 6.3
- [ ] 7.1  [ ] 7.2  [ ] 7.3  [ ] 7.4  [ ] 7.5
- [~] 8
- [ ] 9  [ ] 10  [ ] 11  [ ] 12
- [x] R1  [ ] R2  [~] R3  [ ] R4  [x] R5  [ ] R6  [ ] R7  [ ] R8  [ ] R9  [~] R10  [ ] R11  [ ] R12
- [ ] 14
...
15. **Validación**: servidor y cliente modificados verificados con baterías automatizadas: 33 comprobaciones de servidor (migración, dedup, auth, CORS, Origin, límites, 400/413, run_uuid en pull), 60 escenarios de cliente ejecutando el código real extraído del HTML contra el servidor real (instalación desde cero, actualización con datos duplicados históricos, crear/resolver/cerrar runs, reinicio, sync doble, dos dispositivos, creación offline y reconexión, purga, borrado propagado, review-run protegido, SM-2, token, y reset de DB con cursores huérfanos en localStorage, y colisión de ids servidor/local con remapeo por uuid), y 8 comprobaciones de convivencia cliente antiguo ↔ servidor nuevo ↔ cliente nuevo. Todas en verde.
...
- Cursores dentro de la DB (`sync_meta`) para que datos y cursores vivan y mueran juntos (cierra también el reset parcial no cubierto por la corrección mínima).
